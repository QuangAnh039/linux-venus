...
#include <linux/irqchip/arm-gic-v3_ca.h>
...
#include "irq-gic-common_ca.h"
#include <soc/cortina-access/ca-smc.h>

#define SMC_READL(a)    CA_SMC_CALL_REG_READ(a)
#define SMC_WRITEL(v,a) CA_SMC_CALL_REG_WRITE(a,v)
#define gic_read_typer_ca(c)               CA_SMC_CALL_REG_READ64(c)
#define gic_write_irouter_ca(v, c)         CA_SMC_CALL_REG_WRITE64(c, v)
...
#define gic_data_rdist_rd_base()	(gic_data_rdist()->rd_base_phy)
...
static inline unsigned long gic_dist_base(struct irq_data *d)
{
	switch (get_intid_range(d)) {
	case SGI_RANGE:
	case PPI_RANGE:
	...
	case ESPI_RANGE:
		/* SPI -> dist_base */
		return gic_data.dist_phys_base;
...
static void gic_do_wait_for_rwp(unsigned long base, u32 bit)
{
	u32 count = 1000000;	/* 1s! */

	while (SMC_READL(base + GICD_CTLR) & bit) {
		count--;
		if (!count) {
...
{
	gic_do_wait_for_rwp(gic_data.dist_phys_base, GICD_CTLR_RWP);
}
...
static void gic_enable_redist(bool enable)
{
	...
	val = SMC_READL(rbase + GICR_WAKER);
	if (enable)
		/* Wake up this CPU redistributor */
		val &= ~GICR_WAKER_ProcessorSleep;
	else
		val |= GICR_WAKER_ProcessorSleep;
	SMC_WRITEL(val, rbase + GICR_WAKER);
...
static int gic_peek_irq(struct irq_data *d, u32 offset)
{
	...
	if (gic_irq_in_rdist(d))
		...
		base = gic_data.dist_phys_base;

	return !!(SMC_READL(base + offset + (index / 32) * 4) & mask);
}
...
static void gic_poke_irq(struct irq_data *d, u32 offset)
{
	...
	SMC_WRITEL(mask, base + offset + (index / 32) * 4);
}
...
{
	void __iomem *base = (void __iomem *)gic_dist_base(d);
	u32 offset, index;
	u32 tmp;

	offset = convert_offset_index(d, GICD_IPRIORITYR, &index);

	tmp = SMC_READL(base + index);
	tmp |= prio << (offset * 8);
	SMC_WRITEL(tmp, base + index);
}
...
static void __init gic_dist_init(void)
{
	unsigned int i;
	u64 affinity;
	unsigned long base = gic_data.dist_phys_base;
	u32 val;

	/* Disable the distributor */
	SMC_WRITEL(0, base + GICD_CTLR);
	gic_dist_wait_for_rwp();
	...
	for (i = 32; i < GIC_LINE_NR; i += 32)
		SMC_WRITEL(~0, base + GICD_IGROUPR + i / 8);

	/* Extended SPI range, not handled by the GICv2/GICv3 common code */
	for (i = 0; i < GIC_ESPI_NR; i += 32) {
		SMC_WRITEL(~0U, base + GICD_ICENABLERnE + i / 8);
		SMC_WRITEL(~0U, base + GICD_ICACTIVERnE + i / 8);
	}

	for (i = 0; i < GIC_ESPI_NR; i += 32)
		SMC_WRITEL(~0U, base + GICD_IGROUPRnE + i / 8);

	for (i = 0; i < GIC_ESPI_NR; i += 16)
		SMC_WRITEL(0, base + GICD_ICFGRnE + i / 4);

	for (i = 0; i < GIC_ESPI_NR; i += 4)
		SMC_WRITEL(GICD_INT_DEF_PRI_X4, base + GICD_IPRIORITYRnE + i);
	...
	for (i = 32; i < GIC_LINE_NR; i++)
		gic_write_irouter_ca(affinity, base + GICD_IROUTER + i * 8);

	for (i = 0; i < GIC_ESPI_NR; i++)
		gic_write_irouter_ca(affinity, base + GICD_IROUTERnE + i * 8);
}
...
static int gic_iterate_rdists(int (*fn)(struct redist_region *, void __iomem *))
{
	...
	for (i = 0; i < gic_data.nr_redist_regions; i++) {
		unsigned long ptr = gic_data.redist_regions[i].redist_base_phy;
		u64 typer;
		u32 reg;

		reg = SMC_READL(ptr + GICR_PIDR2) & GIC_PIDR2_ARCH_MASK;
		if (reg != GIC_PIDR2_ARCH_GICv3 &&
		    reg != GIC_PIDR2_ARCH_GICv4) { /* We're in trouble... */
			pr_warn("No redistributor present @%lx\n", ptr);
			break;
		}

		do {
			typer = gic_read_typer_ca(ptr + GICR_TYPER);
			ret = fn(gic_data.redist_regions + i, (void __iomem *)ptr);
			if (!ret)
				return 0;
...
static int __gic_populate_rdist(struct redist_region *region, void __iomem *ptr)
{
	...
	typer = gic_read_typer_ca((unsigned long)ptr + GICR_TYPER);
	if ((typer >> 32) == aff) {
		u64 offset = (u64)ptr - region->redist_base_phy;
		raw_spin_lock_init(&gic_data_rdist()->rd_lock);
		gic_data_rdist_rd_base() = (unsigned long)ptr;
...
static int __gic_update_rdist_properties(struct redist_region *region,
					 void __iomem *ptr)
{
	u64 typer = gic_read_typer_ca(ptr + GICR_TYPER);
	u32 ctlr = SMC_READL(ptr + GICR_CTLR);
...
{
	return SMC_READL(gic_data.dist_phys_base + GICD_CTLR) & GICD_CTLR_DS;
}
...
static int gic_dist_supports_lpis(void)
{
	return (IS_ENABLED(CONFIG_ARM_GIC_V3_ITS) &&
		!!(SMC_READL(gic_data.dist_phys_base + GICD_TYPER) & GICD_TYPER_LPIS) &&
		!gicv3_nolpi);
}
...
static void gic_cpu_init(void)
{
	...
	for (i = 0; i < gic_data.ppi_nr + 16; i += 32)
		SMC_WRITEL(~0, rbase + GICR_IGROUPR0 + i / 8);
				 ...
				 struct fwnode_handle *handle)
{
	 ...
	 */
	typer = SMC_READL(gic_data.dist_phys_base + GICD_TYPER);
	gic_data.rdists.gicd_typer = typer;

	gic_enable_quirks(SMC_READL(gic_data.dist_phys_base + GICD_IIDR),
			  gic_quirks, &gic_data);
	...
	if (!(gic_data.flags & FLAGS_WORKAROUND_CAVIUM_ERRATUM_38539))
		gic_data.rdists.gicd_typer2 = SMC_READL(gic_data.dist_phys_base + GICD_TYPER2);
...
static int __init gic_validate_dist_version(unsigned long dist_base)
{
	u32 reg = SMC_READL(dist_base + GICD_PIDR2) & GIC_PIDR2_ARCH_MASK;
...
static int __init gic_of_init(struct device_node *node, struct device_node *parent)
{
	...
	err = gic_validate_dist_version(dist_phys_base);
	if (err) {
	...
	for (i = 0; i < nr_redist_regions; i++) {
		...
		rdist_regs[i].redist_base_phy = res.start;
			 ...
			 const unsigned long end)
{
	...
	u32 reg = SMC_READL(acpi_data.dist_base + GICD_PIDR2) & GIC_PIDR2_ARCH_MASK;
...
gic_acpi_init(union acpi_subtable_headers *header, const unsigned long end)
{
	...
	err = gic_validate_dist_version((unsigned long)acpi_data.dist_base);
	if (err) {
