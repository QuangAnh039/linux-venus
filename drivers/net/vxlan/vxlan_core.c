...
#ifdef CONFIG_CA_KERNEL_HOOK
		if (!vxlan_addr_multicast(dst) && (ca_kh_hook & CA_KH_HOOK_VXLAN) &&
		    ca_kernel_hook_ops.kho_v4vxlan_add) {
			ca_kernel_hook_ops.kho_v4vxlan_add(0, dev, ndst->dev,
							   local_ip.sin.sin_addr.s_addr,
							   dst->sin.sin_addr.s_addr,
							   src_port, dst_port, vni, tos, ttl);
		}
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
		if (!vxlan_addr_multicast(dst) && (ca_kh_hook & CA_KH_HOOK_VXLAN) &&
		    ca_kernel_hook_ops.kho_v6vxlan_add) {
			ca_kernel_hook_ops.kho_v6vxlan_add(0, dev, ndst->dev,
							   local_ip.sin6.sin6_addr.in6_u.u6_addr32,
							   dst->sin6.sin6_addr.in6_u.u6_addr32,
							   src_port, dst_port, vni, tos, ttl,
							   label);
		}
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if ((ca_kh_hook & CA_KH_HOOK_VXLAN) && ca_kernel_hook_ops.kho_vxlan_open)
		ca_kernel_hook_ops.kho_vxlan_open(0, dev, vxlan->cfg.dst_port, vxlan->cfg.vni);
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if ((ca_kh_hook & CA_KH_HOOK_VXLAN) && ca_kernel_hook_ops.kho_vxlan_delete) {
		if (vxlan->cfg.remote_ip.sa.sa_family == AF_INET6)
			ca_kernel_hook_ops.kho_vxlan_delete(0, dev, AF_INET6);
		else
			ca_kernel_hook_ops.kho_vxlan_delete(0, dev, AF_INET);
	}
#endif
