...
#include <soc/cortina-access/ca_offload_info.h>
#endif
...
void ca_dump_key(struct macsec_key *key)
{
	if (!key)
		return;

	pr_info("  Dump macsec_key:\n");
	print_hex_dump(KERN_INFO, "    id=", DUMP_PREFIX_NONE, 16, 1, key->id, MACSEC_KEYID_LEN,
		       false);
}

void ca_dump_rx_sa(struct macsec_rx_sa *rx_sa)
{
	if (!rx_sa)
		return;

	pr_info("      Dump macsec_rx_sa:\n");
	pr_info("        active=%s, next_pn=%llu\n", BOOL2STR(rx_sa->active), rx_sa->next_pn);
	ca_dump_key(&rx_sa->key);
	if (!rx_sa->sc)
		pr_info("        sc=NULL (rx_sc)\n");
	else
		pr_info("        sc!=NULL (rx_sc)\n");
}

void ca_dump_rx_sc(struct macsec_rx_sc *rx_sc)
{
	if (!rx_sc)
		return;

	pr_info("    Dump macsec_rx_sc:\n");
	pr_info("      sci=0x%016llx, active=%s\n", rx_sc->sci, BOOL2STR(rx_sc->active));
	if (!rx_sc->sa[0]) {
		pr_info("      sa[0]=NULL (rx_sa)\n");
	} else {
		pr_info("      sa[0]!=NULL (rx_sa)\n");
		ca_dump_rx_sa(rx_sc->sa[0]);
	}
	if (!rx_sc->sa[1]) {
		pr_info("      sa[1]=NULL (rx_sa)\n");
	} else {
		pr_info("      sa[1]!=NULL (rx_sa)\n");
		ca_dump_rx_sa(rx_sc->sa[1]);
	}
	if (!rx_sc->sa[2]) {
		pr_info("      sa[2]=NULL (rx_sa)\n");
	} else {
		pr_info("      sa[2]!=NULL (rx_sa)\n");
		ca_dump_rx_sa(rx_sc->sa[2]);
	}
	if (!rx_sc->sa[3]) {
		pr_info("      sa[3]=NULL (rx_sa)\n");
	} else {
		pr_info("      sa[3]!=NULL (rx_sa)\n");
		ca_dump_rx_sa(rx_sc->sa[3]);
	}
	if (!rx_sc->next)
		pr_info("      next=NULL (rx_sc)\n");
}

void ca_dump_tx_sa(struct macsec_tx_sa *tx_sa)
{
	if (!tx_sa)
		return;

	pr_info("      Dump macsec_tx_sa:\n");
	pr_info("        active=%s, next_pn=%llu\n", BOOL2STR(tx_sa->active), tx_sa->next_pn);
	ca_dump_key(&tx_sa->key);
}

void ca_dump_tx_sc(struct macsec_tx_sc *tx_sc)
{
	if (!tx_sc)
		return;

	pr_info("    Dump macsec_tx_sc:\n");
	pr_info("      encoding_sa=%u (AN?)\n", tx_sc->encoding_sa);
	pr_info("      active=%s, encrypt=%s, send_sci=%s, end_station=%s, scb=%s\n",
		BOOL2STR(tx_sc->active), BOOL2STR(tx_sc->encrypt),
		BOOL2STR(tx_sc->send_sci), BOOL2STR(tx_sc->end_station),
		BOOL2STR(tx_sc->scb));

	if (!tx_sc->sa[0]) {
		pr_info("      sa[0]=NULL (tx_sa)\n");
	} else {
		pr_info("      sa[0]!=NULL (tx_sa)\n");
		ca_dump_tx_sa(tx_sc->sa[0]);
	}
	if (!tx_sc->sa[1]) {
		pr_info("      sa[1]=NULL (tx_sa)\n");
	} else {
		pr_info("      sa[1]!=NULL (tx_sa)\n");
		ca_dump_tx_sa(tx_sc->sa[1]);
	}
	if (!tx_sc->sa[2]) {
		pr_info("      sa[2]=NULL (tx_sa)\n");
	} else {
		pr_info("      sa[2]!=NULL (tx_sa)\n");
		ca_dump_tx_sa(tx_sc->sa[2]);
	}
	if (!tx_sc->sa[3]) {
		pr_info("      sa[3]=NULL (tx_sa)\n");
	} else {
		pr_info("      sa[3]!=NULL (tx_sa)\n");
		ca_dump_tx_sa(tx_sc->sa[3]);
	}
}

void ca_dump_secy(struct macsec_secy *secy)
{
	int i;
	...
	pr_info("  Dump macsec_secy:\n");
	if (secy->netdev)
		pr_info("    netdev->name=%s\n", secy->netdev->name);
	pr_info("    n_rx_sc=%u, sci=0x%016llx, key_len=%u, icv_len=%u, validate_frames=%d, replay_window=%u\n",
		secy->n_rx_sc, secy->sci, secy->key_len, secy->icv_len,
		secy->validate_frames, secy->replay_window);
	pr_info("    operational=%s, protect_frames=%s, replay_protect=%s\n",
		BOOL2STR(secy->operational), BOOL2STR(secy->protect_frames),
		BOOL2STR(secy->replay_protect));
	ca_dump_tx_sc(&secy->tx_sc);

	rx_sc = secy->rx_sc;
	for (i = 0; i < secy->n_rx_sc; i++) {
		ca_dump_rx_sc(rx_sc);
		rx_sc = rx_sc->next;
	}
}
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if ((ca_kh_hook & CA_KH_HOOK_MACSEC) &&
	    ca_kernel_hook_ops.kho_macsec_sa_add &&
	    ca_kernel_hook_ops.kho_macsec_sc_add) {
		/* create rx sa */
		ca_kernel_hook_ops.kho_macsec_sa_add(0, dev, 1, macsec_skb_cb(skb)->assoc_num,
						     rx_sa);
		/* create tx sa */
		ca_kernel_hook_ops.kho_macsec_sa_add(0, dev, 0, 0, NULL);
		/* create sc */
		ca_kernel_hook_ops.kho_macsec_sc_add(0, dev, rx_sa);
	}
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
	cp = (unsigned char *)nla_data(tb_sa[MACSEC_SA_ATTR_KEY]);
	real_dev = macsec_priv(dev)->real_dev;

	if (ca_kh_debug & CA_KH_DEBUG_TUNNEL) {
		pr_info("%s: DUMP INFO\n", __func__);
		pr_info("%s: dev->name=%s, real_dev->name=%s\n",
			__func__, dev->name, real_dev->name);
		ca_dump_secy(secy);
		ca_dump_rx_sa(rx_sa);
		snprintf(prefix, sizeof(prefix), "%s: Rx Key=", __func__);
		print_hex_dump(KERN_INFO, prefix, DUMP_PREFIX_NONE, 32, 1, cp, secy->key_len,
			       false);
	}

	if ((ca_kh_hook & CA_KH_HOOK_MACSEC) && ca_kernel_hook_ops.kho_macsec_sa_add)
		memcpy(rx_sa->sa_key, nla_data(tb_sa[MACSEC_SA_ATTR_KEY]), secy->key_len);
#endif
...
static int macsec_add_txsa(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (ca_kh_debug & CA_KH_DEBUG_TUNNEL) {
		pr_info("%s: DUMP INFO\n", __func__);
		pr_info("%s: dev->name=%s, real_dev->name=%s, assoc_num=%u (AN?)\n",
			__func__, dev->name, real_dev->name, assoc_num);
		ca_dump_secy(secy);
		ca_dump_tx_sa(tx_sa);
		snprintf(prefix, sizeof(prefix), "%s: Tx Key=", __func__);
		print_hex_dump(KERN_INFO, prefix, DUMP_PREFIX_NONE, 32, 1, cp, secy->key_len,
			       false);
	}

	if ((ca_kh_hook & CA_KH_HOOK_MACSEC) && ca_kernel_hook_ops.kho_macsec_sa_add)
		memcpy(tx_sa->sa_key, nla_data(tb_sa[MACSEC_SA_ATTR_KEY]), secy->key_len);
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
	/* Create NAT entry before encapsulation. */
	if (ca_kh_hook & CA_KH_HOOK_MACSEC) {
		struct nf_conn *ct = NULL;
		enum ip_conntrack_info ctinfo;
		struct nf_conntrack_tuple *orig_tuple, *reply_tuple;
		unsigned char pf;
		struct ca_packet_info *pkt_info = NULL;

		ct = nf_ct_get(skb, &ctinfo);
		if (!ct) {
			/* may be non-IP packet */
			if (ca_kh_debug & CA_KH_DEBUG_NAT)
				pr_info("%s: NO CT.\n", __func__);
			goto _no_offload;
		}

		if (skb->ca_offload_info.ingress_magic != CA_MAGIC)
			goto _no_offload;

		if (ca_kernel_hook_ops.kho_offload_permit_by_packet_count &&
		    !ca_kernel_hook_ops.kho_offload_permit_by_packet_count(0, skb))
			goto _no_offload;

		/* special handle for policy route */
		if (skb->ca_offload_info.policy_route)
			memset(&skb->ca_offload_info.ipv6_nh, 0,
			       sizeof(skb->ca_offload_info.ipv6_nh));

		pkt_info = &skb->ca_offload_info.ingress_pkt_info;

		if (pkt_info->is_ipv4)
			pf = NFPROTO_IPV4;
		else if (pkt_info->is_ipv6)
			pf = NFPROTO_IPV6;
		else
			goto _no_offload; /* non-IP packet */
		...
		if (ca_kh_debug & CA_KH_DEBUG_NAT) {
			if (pkt_info->is_ipv4) {
				pr_info("%s:%d: orig_tuple, src=%pI4:%u, dst=%pI4:%u\n",
					__func__, __LINE__,
					&orig_tuple->src.u3.ip,
					ntohs(orig_tuple->src.u.all),
					&orig_tuple->dst.u3.ip,
					ntohs(orig_tuple->dst.u.all));
				pr_info("%s:%d: reply_tuple, src=%pI4:%u, dst=%pI4:%u\n",
					__func__, __LINE__,
					&reply_tuple->src.u3.ip,
					ntohs(reply_tuple->src.u.all),
					&reply_tuple->dst.u3.ip,
					ntohs(reply_tuple->dst.u.all));
			} else {
				pr_info("%s:%d: orig_tuple, src=[%pI6c]:%u, dst=[%pI6c]:%u\n",
					__func__, __LINE__,
					&orig_tuple->src.u3.ip,
					ntohs(orig_tuple->src.u.all),
					&orig_tuple->dst.u3.ip,
					ntohs(orig_tuple->dst.u.all));
				pr_info("%s:%d: reply_tuple, src=[%pI6c]:%u, dst=[%pI6c]:%u\n",
					__func__, __LINE__,
					&reply_tuple->src.u3.ip,
					ntohs(reply_tuple->src.u.all),
					&reply_tuple->dst.u3.ip,
		...
		if (ca_kernel_hook_ops.kho_offload_tcp &&
		    pkt_info->ip_prot == IPPROTO_TCP) {
			ca_kernel_hook_ops.kho_offload_tcp(0, skb, pf);
		} else if (ca_kernel_hook_ops.kho_offload_udp &&
			   pkt_info->ip_prot == IPPROTO_UDP) {
			ca_kernel_hook_ops.kho_offload_udp(0, skb, pf);
		} else {
			ca_kernel_hook_ops.kho_offload_generic(0, skb, pf);
		}
	} else {
		/* Must set sw_only. Otherwise, packet may be offloaded
		 * without doing encapsulation.
		 */
		skb->ca_offload_info.sw_only = true;
	}
_no_offload:
#endif /* CONFIG_CA_KERNEL_HOOK */
...
static int macsec_dev_open(struct net_device *dev)
{
	...
	if ((ca_kh_hook & CA_KH_HOOK_MACSEC) && ca_kernel_hook_ops.kho_macsec_open) {
		/* open macsec intf */
		ca_kernel_hook_ops.kho_macsec_open(0, dev);
	}
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if ((ca_kh_hook & CA_KH_HOOK_MACSEC) && ca_kernel_hook_ops.kho_macsec_delete) {
		/* delete macsec intf */
		ca_kernel_hook_ops.kho_macsec_delete(0, dev);
	}
#endif
