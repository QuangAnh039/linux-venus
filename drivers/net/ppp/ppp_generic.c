...
#include "../../../net/l2tp/l2tp_core.h" /* for L2TP private struct */
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
	/* It will point to the PPPoE nexthop directly, bypass LPM. */
	if (skb->ca_offload_info.ingress_magic == CA_MAGIC &&
	    skb->ca_offload_info.policy_route)
		memset(&skb->ca_offload_info.ipv6_nh, 0, sizeof(skb->ca_offload_info.ipv6_nh));
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if (ca_kernel_hook_ops.kho_pppoe_delete)
		ca_kernel_hook_ops.kho_pppoe_delete(0, ppp->dev);
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if (ca_kernel_hook_ops.kho_pppoe_add) {
		struct sock *sk;
		...
		if (sk->sk_protocol == PX_PROTO_OE) {
			/* PPPoE */
			struct net_device *eth_dev;

			eth_dev = po->pppoe_dev;
			ca_kernel_hook_ops.kho_pppoe_add(0, ppp->dev, eth_dev, po);
		} else if (sk->sk_protocol == PX_PROTO_OL2TP) {
			/* PPPoL2TP (L2TPv2) */
			...
			ca_l2tp_cfg.udp6_zero_tx_checksums = tunnel->udp6_zero_tx_checksums;
			ca_l2tp_cfg.udp6_zero_rx_checksums = tunnel->udp6_zero_rx_checksums;
			...
			ca_l2tp_cfg.mtu = ppp->dev->mtu;
			ca_l2tp_cfg.pwtype = session->pwtype;

			ca_kernel_hook_ops.kho_l2tp_add(0, ppp->dev, &ca_l2tp_cfg,
							&tunnel->ca_tunnel_id);
		}
	}
_no_ppp_offload:
