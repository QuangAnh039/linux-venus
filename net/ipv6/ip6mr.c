...
#ifdef CONFIG_CA_KERNEL_HOOK
	if ((ca_kh_hook & CA_KH_HOOK_IP_MULTICAST) &&
	    ca_kernel_hook_ops.kho_ip_mc_vif_del) {
		ca_kernel_hook_ops.kho_ip_mc_vif_del(in6_dev, v, false, vifi);
	}
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if ((ca_kh_hook & CA_KH_HOOK_IP_MULTICAST) &&
	    ca_kernel_hook_ops.kho_ip_mc_vif_add) {
		ca_kernel_hook_ops.kho_ip_mc_vif_add(in6_dev, v, false, vifi);
	}
#endif
