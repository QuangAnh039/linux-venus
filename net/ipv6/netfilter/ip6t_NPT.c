...
ip6t_snpt_tg(struct sk_buff *skb, const struct xt_action_param *par)
{
	...
	bool need_hack_ct = false;
#endif

#ifdef CONFIG_CA_KERNEL_HOOK
	if (skb->ca_offload_info.ct_valid) {
		orig_tuple = &skb->ca_offload_info.ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple;

		/* check if this source NPTv6 is for orig_tuple */
		if (memcmp(&orig_tuple->src.u3.in6, &ipv6_hdr(skb)->saddr, 16) == 0)
			need_hack_ct = true;
	}

	if (ca_kh_debug & CA_KH_DEBUG_NAT) {
		pr_info("npt=0x%p, src_pfx=%pI6c/%u, dst_pfx=%pI6c/%u, need_hack_ct=%u\n",
			npt, &npt->src_pfx.in6, npt->src_pfx_len, &npt->dst_pfx.in6,
			npt->dst_pfx_len, need_hack_ct);
		pr_info("  before: ipv6_src=%pI6c\n", &ipv6_hdr(skb)->saddr);
	}
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if (need_hack_ct && skb->ca_offload_info.ct_valid) {
		/* This NPTv6 module doesn't support netfilter,
		 * so here update ct for correct content in reply_tuple.
		 */
		reply_tuple = &skb->ca_offload_info.ct->tuplehash[IP_CT_DIR_REPLY].tuple;
		reply_tuple->dst.u3.in6 = ipv6_hdr(skb)->saddr;
	}

	if (ca_kh_debug & CA_KH_DEBUG_NAT)
		pr_info("  after: ipv6_src=%pI6c\n", &ipv6_hdr(skb)->saddr);
#endif
...
ip6t_dnpt_tg(struct sk_buff *skb, const struct xt_action_param *par)
{
	...
	if (skb->ca_offload_info.ct_valid) {
		...
		/* check if this dest NPTv6 is for orig_tuple */
		if (memcmp(&orig_tuple->dst.u3.in6, &ipv6_hdr(skb)->daddr, 16) == 0)
			need_hack_ct = true;
	}
	...
	if (need_hack_ct && skb->ca_offload_info.ct_valid) {
		 ...
		 */
		reply_tuple = &skb->ca_offload_info.ct->tuplehash[IP_CT_DIR_REPLY].tuple;
		reply_tuple->src.u3.in6 = ipv6_hdr(skb)->daddr;
	}

	if (ca_kh_debug & CA_KH_DEBUG_NAT)
		pr_info("  after: ipv6_dst=%pI6c\n", &ipv6_hdr(skb)->daddr);
#endif
