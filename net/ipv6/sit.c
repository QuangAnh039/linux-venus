...
static void ipip6_tunnel_unlink(struct sit_net *sitn, struct ip_tunnel *t)
{
	...
	if (ca_kh_debug & CA_KH_DEBUG_TUNNEL) {
		pr_info("%s: 6rd t->dev->name=%s, ifindex=%d, saddr=%pI4, daddr=%pI4\n",
			__func__, t->dev->name, t->dev->ifindex,
			&t->parms.iph.saddr, &t->parms.iph.daddr);
	}

	if ((ca_kh_hook & CA_KH_HOOK_6RD) && ca_kernel_hook_ops.kho_6rd_delete)
		ca_kernel_hook_ops.kho_6rd_delete(0, t->dev, t->tunnel_id);
#endif
}
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if (ca_kh_debug & CA_KH_DEBUG_TUNNEL) {
		pr_info("%s: 6rd dev->name=%s, saddr=%pI4, daddr=%pI4\n",
			__func__, dev->name, &t->parms.iph.saddr,
			&t->parms.iph.daddr);
	}

	if ((ca_kh_hook & CA_KH_HOOK_6RD) && ca_kernel_hook_ops.kho_6rd_add)
		ca_kernel_hook_ops.kho_6rd_add(0, dev->ifindex, t, &t->tunnel_id);
#endif
#endif
}
...
#ifdef CONFIG_CA_KERNEL_HOOK
	/* Create IPv6 hash entry before encapsulation. */
	{
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if (ca_kh_debug & CA_KH_DEBUG_TUNNEL) {
		pr_info("%s: name=%s, prefix=%pI6/%u, relay_prefix=%pI4/%u\n",
			__func__, t->dev->name, &ip6rd->prefix,
