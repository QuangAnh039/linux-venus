...
ip6_tnl_link(struct ip6_tnl_net *ip6n, struct ip6_tnl *t)
{
	...
	if (ca_kh_debug & CA_KH_DEBUG_TUNNEL) {
		if (t->dev)
			pr_info("%s: t->dev: name=%s, type=%d, mac=%pM\n",
				__func__, t->dev->name, t->dev->type, t->dev->dev_addr);
		pr_info("%s: t: i_seqno=%u, o_seqno=%u, tun_hlen=%d, mlink=%d\n",
			__func__, t->i_seqno, atomic_read(&t->o_seqno), t->tun_hlen, t->mlink);
		pr_info("%s: t->parms: name=%s, link=%d, proto=0x%x, hop_limit=%u, flowinfo(be)=0x%08x\n",
			__func__, t->parms.name, t->parms.link, t->parms.proto,
			t->parms.hop_limit, t->parms.flowinfo);
		pr_info("%s: t->parms: flags=0x%08x, i_flags=0x%04x, o_flags=0x%04x, i_key=0x%08x, o_key=0x%08x, fwmark=0x%08x\n",
			__func__, t->parms.flags,
			ntohs(t->parms.i_flags), ntohs(t->parms.o_flags),
			ntohl(t->parms.i_key), ntohl(t->parms.o_key),
			t->parms.fwmark);
		pr_info("%s: t->parms: laddr=%pI6, raddr=%pI6\n",
			__func__, &t->parms.laddr, &t->parms.raddr);
		pr_info("%s: t->encap: type=0x%04x, flags=0x%04x, sport=%u, dport=%u\n",
			__func__, t->encap.type, t->encap.flags,
			ntohs(t->encap.sport), ntohs(t->encap.dport));
	}

	/* DS-Lite is outer IPv6 and inner IP-in-IP */
	if (t->dev && t->parms.proto == IPPROTO_IPIP) {
		if ((ca_kh_hook & CA_KH_HOOK_IP6_TUNNEL) && ca_kernel_hook_ops.kho_ip6tnl_add) {
			ca_kernel_hook_ops.kho_ip6tnl_add(0, t->dev->ifindex, &t->parms,
								&t->tunnel_id);
		}
	}
#endif	
...
ip6_tnl_unlink(struct ip6_tnl_net *ip6n, struct ip6_tnl *t)
{
	...
	for (tp = ip6_tnl_bucket(ip6n, &t->parms);
		...
		if (t == iter) {
#ifdef CONFIG_CA_KERNEL_HOOK
			if (t->dev && t->dev->type == ARPHRD_TUNNEL6) {
				if ((ca_kh_hook & CA_KH_HOOK_IP6_TUNNEL) &&
				    ca_kernel_hook_ops.kho_ip6tnl_delete) {
					if (ca_kh_debug & CA_KH_DEBUG_TUNNEL) {
						pr_info("%s: dev->name=%s, t->tunnel_id=%u\n",
							__func__, t->dev->name, t->tunnel_id);
					}
					ca_kernel_hook_ops.kho_ip6tnl_delete(0, t->dev,
									     t->tunnel_id);
				}
			}
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
	/* Create IPv4 NAT entry before encapsulation. */
		       ...
		       void __user *data, int cmd)
{
	...
	case SIOCDELTUNNEL:
		...
		if (dev->type == ARPHRD_TUNNEL6) {
			if ((ca_kh_hook & CA_KH_HOOK_IP6_TUNNEL) &&
			    ca_kernel_hook_ops.kho_ip6tnl_delete)
				ca_kernel_hook_ops.kho_ip6tnl_delete(0, dev, t->tunnel_id);
		}
#endif
		break;
	default:
