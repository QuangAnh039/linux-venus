...
static int esp6_output(struct xfrm_state *x, struct sk_buff *skb)
{
	...
	*skb_mac_header(skb) = IPPROTO_ESP;
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if ((ca_kh_hook & CA_KH_HOOK_IPSEC) && ca_kernel_hook_ops.kho_ipsec_redirect_to_pe) {
		if (ca_kernel_hook_ops.kho_ipsec_redirect_to_pe(skb, x, __CA_IPV6, 0) == 0) {
			/* Hook function returns 0 means packet is redirected
			 * successfully.
			 *
			 * esp6_output() caller treats returned value as nexthdr
			 * if it is positive.
			 */
			return -EBUSY;
		}
	 ...
	 * without doing encryption/decryption.
	 */
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if ((ca_kh_hook & CA_KH_HOOK_IPSEC) && ca_kernel_hook_ops.kho_ipsec_redirect_to_pe) {
		if (ca_kernel_hook_ops.kho_ipsec_redirect_to_pe(skb, x, __CA_IPV6, 1) == 0) {
			/* Hook function returns 0 means packet is redirected
			 * successfully.
			 *
			 * esp6_input() caller treats returned value as nexthdr
			 * if it is positive.
			 */
			return -EBUSY;
		}
