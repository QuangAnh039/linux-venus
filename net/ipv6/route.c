		  ...
		  struct netlink_ext_ack *extack)
{
	...
	if (err == 0) {
		...
		if (cfg->fc_flags & RTF_GATEWAY) {
			...
			addr = &rt->fib6_dst.addr;
			addr_type = ipv6_addr_type(addr);
		}

		if (ca_kh_debug & CA_KH_DEBUG_ROUTE) {
			pr_info("%s: addr_type=%d, fc_type=%u, fc_dst=%pI6, fc_gateway=%pI6, tb6_id=%u\n",
				__func__, addr_type, cfg->fc_type, &cfg->fc_dst,
				&cfg->fc_gateway, rt->fib6_table->tb6_id);
		}

		/* tb_id 254 is main table; see /etc/iproute2/rt_tables */
		if (rt->fib6_table->tb6_id == 254 &&
		    ((addr_type & (IPV6_ADDR_LOOPBACK | IPV6_ADDR_MULTICAST)) == 0)) {
			/* static route */
			if (ca_kernel_hook_ops.kho_l3_route_add_ipv6_static)
				ca_kernel_hook_ops.kho_l3_route_add_ipv6_static(0,
						rt->fib6_nh->fib_nh_dev, cfg);
		}
	}
...
#ifdef CONFIG_CA_KERNEL_HOOK
static void ca_ip6_route_del(struct fib6_config *cfg,
			     struct fib6_info *rt)
{
	const struct in6_addr *addr;
	int addr_type;

	if (!cfg || !rt || !rt->fib6_table)
		return;

	/* RTF_LOCAL is local route to intf 'lo' */
	if (!(cfg->fc_flags & RTF_LOCAL)) {
		...
		if (ca_kh_debug & CA_KH_DEBUG_ROUTE) {
			pr_info("%s: rt->fib6_nh->fib_nh_dev->name=%s, cfg->ifindex=%d\n",
				__func__, rt->fib6_nh->fib_nh_dev->name, cfg->fc_ifindex);
			pr_info("%s: addr_type=%d, rt6i_dst=%pI6c/%d, rt6i_gateway=%pI6c, tb6_id=%u\n",
				__func__, addr_type, &cfg->fc_dst,
				cfg->fc_dst_len, &cfg->fc_gateway,
				rt->fib6_table->tb6_id);
			pr_info("%s: cfg->fc_protocol=%u\n", __func__,
				cfg->fc_protocol); /* RTPROT_KERNEL */
			pr_info("%s: cfg->fc_flags=0x%08x\n", __func__,
				cfg->fc_flags); /* RTF_UP, RTF_GATEWAY, .. */
		}
		...
		if (rt->fib6_table->tb6_id == 254 &&
			...
			/* static route */
			if (ca_kernel_hook_ops.kho_l3_route_del_ipv6_static)
				ca_kernel_hook_ops.kho_l3_route_del_ipv6_static(0,
						cfg->fc_ifindex, &cfg->fc_dst, cfg->fc_dst_len,
						&cfg->fc_gateway);
		}
	}
}

static int __ca_ip6_del_rt(struct fib6_info *rt, struct fib6_config *cfg)
{
	struct net *net = cfg->fc_nlinfo.nl_net;
	struct fib6_table *table;
	int err;
