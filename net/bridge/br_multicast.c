...
#if IS_ENABLED(CONFIG_CA_KERNEL_HOOK)
...
#if IS_ENABLED(CONFIG_CA_KERNEL_HOOK)
	if ((ca_kh_hook & CA_KH_HOOK_BRIDGE_MULTICAST) &&
	    ca_kernel_hook_ops.kho_br_ipa_mdb_del)
		ca_kernel_hook_ops.kho_br_ipa_mdb_del(mp);
#endif
...
#if IS_ENABLED(CONFIG_CA_KERNEL_HOOK)
	new_group = !rcu_access_pointer(mp->ports);
#endif
...
#if IS_ENABLED(CONFIG_CA_KERNEL_HOOK)
	if (new_group && (ca_kh_hook & CA_KH_HOOK_BRIDGE_MULTICAST) &&
	    ca_kernel_hook_ops.kho_br_ipa_mdb_add)
		ca_kernel_hook_ops.kho_br_ipa_mdb_add(mp);
#endif
...
#if IS_ENABLED(CONFIG_CA_KERNEL_HOOK)
		if (ca_kh_hook & CA_KH_HOOK_BRIDGE_MULTICAST) {
			pr_warn_once("net_bridge %s BROPT_MULTICAST_ENABLED is not enabled!\n",
				     (*brmctx)->br->dev->name);
		}
#endif
		return 0;
