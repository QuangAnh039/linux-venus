...
static int add_del_if(struct net_bridge *br, int ifindex, int isadd)
{
	...
	if (ca_kernel_hook_ops.kho_l3_intf_add && ca_kernel_hook_ops.kho_l3_intf_delete) {
		ca_kernel_hook_ops.kho_l3_intf_delete(0, br->dev);
#if defined(CONFIG_ARCH_CA_MERCURY)
		if (br->dev->flags & IFF_UP)
			ca_kernel_hook_ops.kho_l3_intf_add(0, br->dev, &ip_addr, 0);
#else
		if (br->dev->flags & IFF_UP)
			ca_kernel_hook_ops.kho_l3_intf_add(0, br->dev, &ip_addr, 0, 0);
#endif
	}
