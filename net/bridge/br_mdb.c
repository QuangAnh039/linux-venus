		   ...
		   int type)
{
	...
	case RTM_NEWMDB:
		if ((ca_kh_hook & CA_KH_HOOK_BRIDGE_MULTICAST) &&
		    ca_kernel_hook_ops.kho_br_ipa_port_mdb_add)
			ca_kernel_hook_ops.kho_br_ipa_port_mdb_add(mp, pg);
		break;
	case RTM_DELMDB:
		if ((ca_kh_hook & CA_KH_HOOK_BRIDGE_MULTICAST) &&
		    ca_kernel_hook_ops.kho_br_ipa_port_mdb_del)
			ca_kernel_hook_ops.kho_br_ipa_port_mdb_del(mp, pg);
		break;
	}
#endif
...
#if IS_ENABLED(CONFIG_CA_KERNEL_HOOK)
	if (!rcu_access_pointer(mp->ports)) {
		if ((ca_kh_hook & CA_KH_HOOK_BRIDGE_MULTICAST) &&
