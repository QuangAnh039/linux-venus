...
#ifdef CONFIG_CA_KERNEL_HOOK
void call_kho_l2_br_add_del_if(struct net_bridge *br, struct net_bridge_port *p)
{
	int i;
	int count = 0;
	struct __net_bridge_port br_ports; /* list head */
	struct __net_bridge_port *p_br_port = NULL;
	struct net_bridge_port *iter;
	bool is_add = false;

	if (ca_kernel_hook_ops.kho_l2_br_add_del_if) {
		INIT_LIST_HEAD(&br_ports.list);

		/* Get total number of bridge member ports
		 * no matter member port is up or not.
		 */
		list_for_each_entry(iter, &br->port_list, list) {
			count++;

			if (iter->dev == p->dev) {
				is_add = true;

#if defined(CONFIG_ARCH_CA_MERCURY) && !defined(CONFIG_NET_SWITCHDEV)
				/* Disable mcast and bcast flooding
				 * for interface-per-port design
				 * when adding port into bridge.
				 */
				if (p->dev->ca_intf_id_valid) {
		...
		if (count) {
			p_br_port = kmalloc_array(count, sizeof(struct __net_bridge_port),
						  GFP_KERNEL);
			if (!p_br_port)
				return;
			...
			i = 0;
			list_for_each_entry(iter, &br->port_list, list) {
				p_br_port[i].dev = iter->dev;
				list_add_rcu(&p_br_port[i].list, &br_ports.list);
				i++;
			}
		}
		ca_kernel_hook_ops.kho_l2_br_add_del_if(0, br->dev, &br_ports, is_add, p->dev);

		kfree(p_br_port);
	}
}

/* get the member dev in br_dev, and returned them in dev_list */
void ca_get_br_devlist(struct net_device *br_dev, struct net_device **dev_list,
		       int num)
{
...
EXPORT_SYMBOL(ca_get_br_devlist);
...
#ifdef CONFIG_CA_KERNEL_HOOK
	call_kho_l2_br_add_del_if(br, p);
#endif
