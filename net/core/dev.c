...
struct kernel_hook_ops ca_kernel_hook_ops = {0};
EXPORT_SYMBOL(ca_kernel_hook_ops);
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
/**
 *	netif_rx_cortina - post buffer to the network code (Cortina SoC only)
 *	@skb: buffer to post
 *
...
int netif_rx_cortina(struct sk_buff *skb)
{
	int ret;
...
EXPORT_SYMBOL(netif_rx_cortina);
				    ...
				    struct packet_type **ppt_prev)
{
	...
	if (!(skb->dev->priv_flags & IFF_EBRIDGE)) {
		char buf[6]; /* longest string is ".4096" */
		struct ca_packet_info *pkt_info;

		pkt_info = &skb->ca_offload_info.ingress_pkt_info;

		snprintf(skb->br_member_dev_name, IFNAMSIZ, "%s", skb->dev->name);

		if (skb->ca_offload_info.ingress_magic == CA_MAGIC) {
			if (skb->ca_offload_info.ingress_pkt_info.first_tag) {
				snprintf(buf, 6, ".%u", ntohs(pkt_info->first_vid) & VLAN_VID_MASK);
				strcat(skb->br_member_dev_name, buf);
			}

			if (skb->ca_offload_info.ingress_pkt_info.second_tag) {
				snprintf(buf, 6, ".%u",
					 ntohs(pkt_info->second_vid) & VLAN_VID_MASK);
				strcat(skb->br_member_dev_name, buf);
			}
		}
	}
	...
	if (ca_kh_debug & CA_KH_DEBUG_ARP)
		pr_info("%s: skb @ %p, skb->dev->name=%s, priv_flags=0x%llx, br_member_dev_name=%s\n",
			__func__, skb, skb->dev->name, skb->dev->priv_flags,
			skb->br_member_dev_name);
#endif
