...
#ifdef CONFIG_CA_KERNEL_HOOK
			if (ca_kernel_hook_ops.kho_neigh_update_neighbour)
				ca_kernel_hook_ops.kho_neigh_update_neighbour(0, n);
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if (ca_kernel_hook_ops.kho_neigh_update_neighbour)
		ca_kernel_hook_ops.kho_neigh_update_neighbour(0, neigh);
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
void call_kho_neigh_add(struct neighbour *neigh, u8 *lladdr, u8 new_state)
{
	struct neigh_table *tbl;
	struct __ca_ip_address ip_addr = {0};
	int ret;

	if (!ca_kernel_hook_ops.kho_neigh_add)
		return;
	...
	if (tbl->key_len == 4) {
		ip_addr.afi = __CA_IPV4;
		ip_addr.ip_addr.ipv4_addr = ntohl(*((u32 *)neigh->primary_key));
		ip_addr.addr_len = 32;

		if (ca_kh_debug & CA_KH_DEBUG_ARP) {
			pr_info("%s: ADD ipv4_addr=%pI4, new_state=0x%02x\n",
				__func__, neigh->primary_key, new_state);
		}
	} else {
		__be32 *ipv6_addr = (__be32 *)neigh->primary_key;

		ip_addr.afi = __CA_IPV6;
		ip_addr.ip_addr.ipv6_addr[0] = ntohl(*ipv6_addr);
		ip_addr.ip_addr.ipv6_addr[1] = ntohl(*(ipv6_addr + 1));
		ip_addr.ip_addr.ipv6_addr[2] = ntohl(*(ipv6_addr + 2));
		ip_addr.ip_addr.ipv6_addr[3] = ntohl(*(ipv6_addr + 3));
		ip_addr.addr_len = 128;

		if (ca_kh_debug & CA_KH_DEBUG_ARP) {
			pr_info("%s: ADD ipv6_addr=%pI6, new_state=0x%02x\n",
				__func__, neigh->primary_key, new_state);
		}
	}

	ret = ca_kernel_hook_ops.kho_neigh_add(0, neigh->dev,
					       (u8 *)lladdr, &ip_addr, neigh->child_ifindex,
					       (new_state & NUD_PERMANENT) != 0);
	if (ret == 14) { /* CA_E_EXISTS */
		ca_kernel_hook_ops.kho_neigh_update(0, neigh->dev,
						    (u8 *)lladdr, &ip_addr,
						    neigh->child_ifindex,
						    (new_state & NUD_PERMANENT) != 0);
	}
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if (lladdr && dev->addr_len == 6 && neigh->ha[0] == 0 &&
	    neigh->ha[1] == 0 && neigh->ha[2] == 0 && neigh->ha[3] == 0 &&
	    neigh->ha[4] == 0 && neigh->ha[5] == 0)
		call_kho_neigh_add(neigh, (u8 *)lladdr, new);
#endif
...
out:
	...
	if (!(new & NUD_VALID)) {
		...
		if (tbl->key_len == 4) {
			...
			memcpy(&in_addr, neigh->primary_key, sizeof(struct in_addr));

			if (ca_kernel_hook_ops.kho_nat_entry_delete)
				ca_kernel_hook_ops.kho_nat_entry_delete(0, &in_addr);
			...
			if (ca_kh_debug & CA_KH_DEBUG_ARP) {
				pr_info("%s: DEL ipv4_addr=%pI4, new=0x%02x, flags=0x%08x\n",
					__func__, neigh->primary_key, new, flags);
			}
		} else {
			struct in6_addr in6_addr;

			memcpy(&in6_addr, neigh->primary_key, sizeof(struct in6_addr));

			if (ca_kernel_hook_ops.kho_nat6_entry_delete)
				ca_kernel_hook_ops.kho_nat6_entry_delete(0, &in6_addr);

			ip_addr.afi = __CA_IPV6;
			ip_addr.ip_addr.ipv6_addr[0] = ntohl(in6_addr.s6_addr32[0]);
			ip_addr.ip_addr.ipv6_addr[1] = ntohl(in6_addr.s6_addr32[1]);
			ip_addr.ip_addr.ipv6_addr[2] = ntohl(in6_addr.s6_addr32[2]);
			ip_addr.ip_addr.ipv6_addr[3] = ntohl(in6_addr.s6_addr32[3]);
			ip_addr.addr_len = 128;

			if (ca_kh_debug & CA_KH_DEBUG_ARP) {
				pr_info("%s: DEL ipv6_addr=%pI6, new=0x%02x, flags=0x%08x\n",
					__func__, neigh->primary_key, new, flags);
			}
		}
		if (ca_kernel_hook_ops.kho_neigh_delete) {
			ca_kernel_hook_ops.kho_neigh_delete(0, neigh->dev, &ip_addr,
				(old & NUD_PERMANENT) != 0);
		}
	} else {
		if (lladdr && dev->addr_len == 6) {
			...
			if (tbl->key_len == 4) {
				...
				if (ca_kh_debug & CA_KH_DEBUG_ARP) {
					pr_info("%s: UPDATE ipv4_addr=%pI4, new=0x%02x, flags=0x%08x\n",
						__func__, neigh->primary_key, new, flags);
				}
			} else {
				struct in6_addr in6_addr;
				...
				if (ca_kh_debug & CA_KH_DEBUG_ARP) {
					pr_info("%s: UPDATE ipv6_addr=%pI6, new=0x%02x, flags=0x%08x\n",
						__func__, neigh->primary_key, new, flags);
				}
			}

			if (ca_kernel_hook_ops.kho_neigh_update) {
				ca_kernel_hook_ops.kho_neigh_update(0, dev,
					(u8 *)lladdr, &ip_addr, neigh->child_ifindex,
					(new & NUD_PERMANENT) != 0);
			}
		}
...
int neigh_get_child_ifindex_by_skb(struct sk_buff *skb)
{
	...
	/* if it's a tag frame, get the real ifindex for member of bridge */
	if ((skb->dev->priv_flags & IFF_EBRIDGE) && skb->pkt_type != PACKET_LOOPBACK) {
		tmp_dev = first_net_device(&init_net);
		if (ca_kh_debug & CA_KH_DEBUG_ARP)
			pr_info("%s: skb %s\n", __func__, skb->br_member_dev_name);
		while (tmp_dev) {
			if (ca_kh_debug & CA_KH_DEBUG_ARP) {
				pr_info("%s: found [%s] ifindex %d\n",
					__func__, tmp_dev->name, tmp_dev->ifindex);
			}

			/* the br_member_dev_name has been stored in __netif_receive_skb_core() */
			if (!strcmp(tmp_dev->name, skb->br_member_dev_name)) {
				if (ca_kh_debug & CA_KH_DEBUG_ARP) {
					pr_info("%s: Got the net device of %s\n",
						__func__, tmp_dev->name);
					pr_info("%s: ifindex=%d\n", __func__, tmp_dev->ifindex);
				}
				return tmp_dev->ifindex;
			}
			tmp_dev = next_net_device(tmp_dev);
		}
		pr_info("%s: Why cannot find child_ifindex with dev %s??\n",
			__func__, skb->br_member_dev_name);
		/*dump ifindex...*/
		tmp_dev = first_net_device(&init_net);
		while (tmp_dev) {
			pr_info("\t dump [%s] ifindex %d\n", tmp_dev->name, tmp_dev->ifindex);
			tmp_dev = next_net_device(tmp_dev);
		}
	}
	return skb->skb_iif;
}
EXPORT_SYMBOL(neigh_get_child_ifindex_by_skb);
				 ...
				 struct sk_buff *skb)
{
	...
	if (neigh) {
		/* Set the child_ifindex into struct neighbour for downstream
		 * vlan fast path.
		 */
		neigh->child_ifindex = neigh_get_child_ifindex_by_skb(skb);
		neigh_update(neigh, lladdr, NUD_STALE, NEIGH_UPDATE_F_OVERRIDE, 0);
	}
	return neigh;
}
EXPORT_SYMBOL(neigh_event_ns);
#else /* !CONFIG_CA_KERNEL_HOOK */
struct neighbour *neigh_event_ns(struct neigh_table *tbl,
				 u8 *lladdr, void *saddr,
				 struct net_device *dev)
{
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if (neigh->nud_state == NUD_STALE && neigh->dev->addr_len == 6 &&
	    (neigh->ha[0] != 0 || neigh->ha[1] != 0 ||
	     neigh->ha[2] != 0 || neigh->ha[3] != 0 ||
	     neigh->ha[4] != 0 || neigh->ha[5] != 0))
		call_kho_neigh_add(neigh, neigh->ha, neigh->nud_state);
#endif
