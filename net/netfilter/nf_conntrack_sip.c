...
static void __exit nf_conntrack_sip_fini(void)
{
	...
	for (i = 0; i < ports_c; i++) {
		if (ca_kernel_hook_ops.kho_bypass_tcp_portlist_delete)
			ca_kernel_hook_ops.kho_bypass_tcp_portlist_delete(0, ports[i],
									  CA_BYPASS_CNT_VALID);

		if (ca_kernel_hook_ops.kho_bypass_udp_portlist_delete)
			ca_kernel_hook_ops.kho_bypass_udp_portlist_delete(0, ports[i],
									  CA_BYPASS_CNT_VALID);
	}
#endif
...
static int __init nf_conntrack_sip_init(void)
{
	...
	for (i = 0; i < ports_c; i++) {
		if (ca_kernel_hook_ops.kho_bypass_tcp_portlist_add)
			ca_kernel_hook_ops.kho_bypass_tcp_portlist_add(0, ports[i],
								       CA_BYPASS_CNT_VALID);

		if (ca_kernel_hook_ops.kho_bypass_udp_portlist_add)
			ca_kernel_hook_ops.kho_bypass_udp_portlist_add(0, ports[i],
								       CA_BYPASS_CNT_VALID);
	}
#endif
	return 0;
