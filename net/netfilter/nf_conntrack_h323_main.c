			   ...
			   H245_TransportAddress *taddr)
{
	...
	if (ret == 0) {
		/* RTP */
		if (ca_kernel_hook_ops.kho_bypass_tcp_portlist_add) {
			ca_kernel_hook_ops.kho_bypass_udp_portlist_add(0, ntohs(rtp_port),
								       CA_BYPASS_CNT_VALID);
		}

		/* RTCP */
		if (ca_kernel_hook_ops.kho_bypass_tcp_portlist_add) {
			ca_kernel_hook_ops.kho_bypass_udp_portlist_add(0, ntohs(rtcp_port),
								       CA_BYPASS_CNT_VALID);
		}
	}
#endif
		     ...
		     struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	if (ca_kernel_hook_ops.kho_bypass_tcp_portlist_add) {
		ca_kernel_hook_ops.kho_bypass_tcp_portlist_add(0, ntohs(th->source),
							       CA_BYPASS_CNT_VALID);
		ca_kernel_hook_ops.kho_bypass_tcp_portlist_add(0, ntohs(th->dest),
							       CA_BYPASS_CNT_VALID);
	}

	/* No idea why I have to delete opposite direction, but it works! */
	if (ca_kernel_hook_ops.kho_nat_entry_session_delete) {
		ca_kernel_hook_ops.kho_nat_entry_session_delete(0, SOL_TCP,
				iph->daddr, iph->saddr, th->dest, th->source);
	}
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
	/* Q.931 TCP */
	if (ca_kernel_hook_ops.kho_bypass_tcp_portlist_delete) {
		ca_kernel_hook_ops.kho_bypass_tcp_portlist_delete(0, Q931_PORT,
								  CA_BYPASS_CNT_VALID);
	}

	/* RAS UDP */
	if (ca_kernel_hook_ops.kho_bypass_udp_portlist_delete) {
		ca_kernel_hook_ops.kho_bypass_udp_portlist_delete(0, RAS_PORT,
								  CA_BYPASS_CNT_VALID);
	}
#endif
}
