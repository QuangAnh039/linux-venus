...
static void ca_bypass_udp_portlist_check(struct nf_conn *ct,
					 struct sk_buff *skb,
					 unsigned int dataoff,
					 enum ip_conntrack_info ctinfo)
{
	...
	if (skb->ca_offload_info.ingress_magic != CA_MAGIC ||
	    !skb->dev || strcmp(skb->dev->name, "lo") == 0)
		return;

	orig_tuple = &ct->tuplehash[IP_CT_DIR_ORIGINAL].tuple;
	reply_tuple = &ct->tuplehash[IP_CT_DIR_REPLY].tuple;

	if (ca_kernel_hook_ops.kho_bypass_udp_portlist_exist) {
		if (hdr && ca_kernel_hook_ops.kho_bypass_udp_portlist_exist(ntohs(hdr->source))) {
			skb->ca_offload_info.sw_only = true;
			return;
		}

		if (hdr && ca_kernel_hook_ops.kho_bypass_udp_portlist_exist(ntohs(hdr->dest))) {
			skb->ca_offload_info.sw_only = true;
			return;
		}

		if (ca_kernel_hook_ops.kho_bypass_udp_portlist_exist
		    (ntohs(orig_tuple->src.u.udp.port))) {
			skb->ca_offload_info.sw_only = true;
			return;
		}

		if (ca_kernel_hook_ops.kho_bypass_udp_portlist_exist
		    (ntohs(orig_tuple->dst.u.udp.port))) {
			skb->ca_offload_info.sw_only = true;
			return;
		}

		if (ca_kernel_hook_ops.kho_bypass_udp_portlist_exist
		    (ntohs(reply_tuple->src.u.udp.port))) {
			skb->ca_offload_info.sw_only = true;
			return;
		}

		if (ca_kernel_hook_ops.kho_bypass_udp_portlist_exist
		    (ntohs(reply_tuple->dst.u.udp.port))) {
			skb->ca_offload_info.sw_only = true;
			return;
		}
	}
...
#ifdef CONFIG_CA_KERNEL_HOOK
	ca_ct_backup(ct, skb);
	ca_bypass_udp_portlist_check(ct, (struct sk_buff *)skb, dataoff, ctinfo);
#endif
