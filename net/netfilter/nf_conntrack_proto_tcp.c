...
static void ca_bypass_tcp_portlist_check(struct nf_conn *ct,
					 struct sk_buff *skb,
					 unsigned int dataoff,
					 enum ip_conntrack_info ctinfo)
{
	...
	/* if the flag sw_only is already set, just return */
	if (skb->ca_offload_info.sw_only)
		return;
	...
	if (ca_kh_debug & CA_KH_DEBUG_BYPASS_PORTLIST)
		pr_info("%s: th->soucre=%d th->dest=%d\n",
			__func__, ntohs(th->source), ntohs(th->dest));

	if (skb->ca_offload_info.ingress_magic == CA_MAGIC &&
	    ca_kernel_hook_ops.kho_bypass_tcp_portlist_exist) {
		if (ca_kernel_hook_ops.kho_bypass_tcp_portlist_exist(ntohs(th->source)))
			skb->ca_offload_info.sw_only = true;
		if (ca_kernel_hook_ops.kho_bypass_tcp_portlist_exist(ntohs(th->dest)))
			skb->ca_offload_info.sw_only = true;
		if (ca_kernel_hook_ops.kho_bypass_tcp_portlist_exist(ntohs(tuple->src.u.tcp.port)))
			skb->ca_offload_info.sw_only = true;
		if (ca_kernel_hook_ops.kho_bypass_tcp_portlist_exist(ntohs(tuple->dst.u.tcp.port)))
			skb->ca_offload_info.sw_only = true;
	}
}
...
#ifdef CONFIG_CA_KERNEL_HOOK
	ca_ct_backup(ct, skb);
	ca_bypass_tcp_portlist_check(ct, (struct sk_buff *)skb, dataoff, ctinfo);
#endif
