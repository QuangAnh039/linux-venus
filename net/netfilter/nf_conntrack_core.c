...
#ifdef CONFIG_CA_KERNEL_HOOK
	/* Without this hook, NAT entries can't be removed totally
	 * in repeated add-delete massive-session test.
	 */
	struct nf_conntrack_tuple *tuple0 = &ct->tuplehash[0].tuple;
	struct nf_conntrack_tuple *tuple1 = &ct->tuplehash[1].tuple;
	int ret;

	if (test_bit(IPS_CONFIRMED_BIT, &ct->status)) {
		if (ca_kh_debug & CA_KH_DEBUG_NAT) {
			if (tuple0->src.l3num == AF_INET) {
				pr_info("%s: tuple[0], src=%pI4:%u, dst=%pI4:%u, ip_prot=%u\n",
					__func__,
					&tuple0->src.u3.ip, ntohs(tuple0->src.u.all),
					&tuple0->dst.u3.ip, ntohs(tuple0->dst.u.all),
					tuple0->dst.protonum);
			} else {
				pr_info("%s: tuple[0], src=[%pI6c]:%u, dst=[%pI6c]:%u, ip_prot=%u\n",
					__func__,
					&tuple0->src.u3.ip6, ntohs(tuple0->src.u.all),
					&tuple0->dst.u3.ip6, ntohs(tuple0->dst.u.all),
					tuple0->dst.protonum);
			}
		}

		if (ca_kernel_hook_ops.kho_nat_entry_session_delete_force)
			ret = ca_kernel_hook_ops.kho_nat_entry_session_delete_force(0, tuple0);

		if (tuple1) {
			if (ca_kh_debug & CA_KH_DEBUG_NAT) {
				if (tuple1->src.l3num == AF_INET) {
					pr_info("%s: tuple[1], src=%pI4:%u, dst=%pI4:%u, ip_prot=%u\n",
						__func__,
						&tuple1->src.u3.ip, ntohs(tuple1->src.u.all),
						&tuple1->dst.u3.ip, ntohs(tuple1->dst.u.all),
						tuple1->dst.protonum);
				} else {
					pr_info("%s: tuple[1], src=[%pI6c]:%u, dst=[%pI6c]:%u, ip_prot=%u\n",
						__func__,
						&tuple1->src.u3.ip6, ntohs(tuple1->src.u.all),
						&tuple1->dst.u3.ip6, ntohs(tuple1->dst.u.all),
						tuple1->dst.protonum);
				}
			}

			if (ca_kernel_hook_ops.kho_nat_entry_session_delete_force) {
				ret = ca_kernel_hook_ops.kho_nat_entry_session_delete_force(0,
											    tuple1);
			}
		}
	}
...
#ifdef CONFIG_CA_KERNEL_HOOK
	struct nf_conntrack_tuple *tuple0;
...
#ifdef CONFIG_CA_KERNEL_HOOK
		/* triggered when number of conntrack exceeds nf_conntrack_max */
		tuple0 = &tmp->tuplehash[0].tuple;
		tuple1 = &tmp->tuplehash[1].tuple;
...
#ifdef CONFIG_CA_KERNEL_HOOK
#ifdef CONFIG_ARCH_CA_MERCURY
void ca_nat_timer_refresh(struct nf_conn *ct)
{
	struct nf_conntrack_tuple *tuple0, *tuple1;
	struct net *net = nf_ct_net(ct);
	struct timespec64 now, last_hit_time, elapsed_time;
	int ret;
	s32 newtime;
	...
	if (!ca_kernel_hook_ops.kho_nat_entry_hit_time_refresh ||
	    !ca_kernel_hook_ops.kho_nat_entry_session_delete_force)
		return;

	tuple0 = &ct->tuplehash[0].tuple;
	tuple1 = &ct->tuplehash[1].tuple;

	if (test_bit(IPS_CONFIRMED_BIT, &ct->status)) {
		/* Once TCP flow already enters FIN_WAIT state which is triggered by TCP FIN/ACK
		 * packet, netfilter waits for TCP ACK packets to change state.
		 * When TCP flow already enters CLOSE state which is triggered by TCP RST packet,
		 * TCP flow will be expired soon.
		 * When above cases occur, nf_ct_deleted() shouldn't be skipped, and it is
		 * unnecessary to refresh HW NAT aging time.
		 */
		if (tuple0->dst.protonum == IPPROTO_TCP &&
		    ct->proto.tcp.state >= TCP_CONNTRACK_FIN_WAIT)
			return;

		ret = ca_kernel_hook_ops.kho_nat_entry_hit_time_refresh(0, tuple0, tuple1,
									&last_hit_time);
		if (ret == 0) {
			ktime_get_ts64(&now);
			elapsed_time = timespec64_sub(now, last_hit_time);

			if (elapsed_time.tv_sec < 0) /* possible? */
				return;

			/* HW NAT entry is still working.
			 * Extend the aging time of this ct.
			 */
			if (tuple0->dst.protonum == IPPROTO_TCP) {
				tn = &net->ct.nf_ct_proto.tcp;
				newtime = nfct_time_stamp +
					  tn->timeouts[ct->proto.tcp.state] -
					  elapsed_time.tv_sec * HZ;
			} else if (tuple0->dst.protonum == IPPROTO_UDP) {
				/* Conntrack status of UDP packet must
				 * be UDP_CT_REPLIED because
				 * IPS_SRC_NAT_DONE_BIT &
				 * IPS_DST_NAT_DONE_BIT are set.
				 */
				un = &net->ct.nf_ct_proto.udp;
				newtime = nfct_time_stamp +
					  un->timeouts[UDP_CT_REPLIED] -
					  elapsed_time.tv_sec * HZ;
			} else {
				/* There are other registered protocols
				 * and different timeout values.
				 * Usually, those protocols are not
				 * offloaded.
				 * Therefore, unless it's important to
				 * distinguish their timeout values,
				 * only use generic timeout value for
				 * simple design.
				 */
				gn = &net->ct.nf_ct_proto.generic;
				newtime = nfct_time_stamp +
					  gn->timeout -
					  elapsed_time.tv_sec * HZ;
			}

			if (newtime >= (s32)ct->timeout)
				ct->timeout = newtime;
		}

		if (nf_ct_is_expired(ct)) {
			if (ca_kh_debug & CA_KH_DEBUG_NAT) {
				pr_info("%s: Delete expired NAT entry:\n", __func__);
			...
			ca_kernel_hook_ops.kho_nat_entry_session_delete_force(0, tuple0);
			ca_kernel_hook_ops.kho_nat_entry_session_delete_force(0, tuple1);
		}
	}
}
EXPORT_SYMBOL(ca_nat_timer_refresh);
#else /* !CONFIG_ARCH_CA_MERCURY */
void ca_nat_timer_refresh(struct nf_conn *ct)
{
	struct nf_conntrack_tuple *tuple0, *tuple1;
	struct net *net = nf_ct_net(ct);
	...
	/* 10-second buffer to refresh timer */
	if ((s32)(ct->timeout - nfct_time_stamp) > 10 * HZ)
		return;

	if (!ca_kernel_hook_ops.kho_nat_entry_timer_refresh)
		return;
	...
	if (test_bit(IPS_CONFIRMED_BIT, &ct->status)) {
		...
		ret = ca_kernel_hook_ops.kho_nat_entry_timer_refresh(0, tuple0, tuple1,
								     &aging_time);
		if (ret == 0) {
			/* HW NAT entry is still working.
			 * Extend the aging time of this ct.
			...
			if (tuple0->dst.protonum == IPPROTO_TCP) {
				tn = &net->ct.nf_ct_proto.tcp;
				newtime = nfct_time_stamp +
					min(tn->timeouts[ct->proto.tcp.state],
					    aging_time * HZ);
			} else if (tuple0->dst.protonum == IPPROTO_UDP) {
				/* Conntrack status of UDP packet must
				 * be UDP_CT_REPLIED because
				...
				un = &net->ct.nf_ct_proto.udp;
				newtime = nfct_time_stamp +
					min(un->timeouts[UDP_CT_REPLIED],
					    aging_time * HZ);
			} else {
				/* There are other registered protocols
				 * and different timeout values.
				 ...
				 */
				gn = &net->ct.nf_ct_proto.generic;
				newtime = nfct_time_stamp + min(gn->timeout, aging_time * HZ);
			}
			if (newtime >= (s32)ct->timeout)
				ct->timeout = newtime;

			/* Do NOT delete conntrack;
			 * HW NAT is still working.
			 */
			return;
		}
	}
...
#ifdef CONFIG_CA_KERNEL_HOOK
static void ca_ct_backup(struct nf_conn *ct, struct sk_buff *skb)
{
	if (skb->ca_offload_info.ingress_magic == CA_MAGIC) {
		skb->ca_offload_info.ct = ct;
		skb->ca_offload_info.ct_valid = true;
	}
}
#endif
...
#ifdef CONFIG_CA_KERNEL_HOOK
static void nf_cleanup_call_kho(struct nf_conn *ct)
{
	struct nf_conntrack_tuple *tuple0, *tuple1;

	if (!ct || !ca_kernel_hook_ops.kho_nat_entry_session_delete_force)
		return;
...
#ifdef CONFIG_CA_KERNEL_HOOK
		/* triggered by command "conntrack -F" */
		if (test_bit(IPS_CONFIRMED_BIT, &ct->status))
			nf_cleanup_call_kho(ct);
#endif
