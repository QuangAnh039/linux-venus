...
void xfrm_state_find_by_xfrm_policy(struct xfrm_policy *pol, struct xfrm_state **x1)
{
	...
	if (!pol || pol->xfrm_nr == 0 || pol->action != XFRM_POLICY_ALLOW)
		return;
	...
	hlist_for_each_entry(x, net->xfrm.state_bydst + h, bydst) {
		if (x->props.family == encap_family &&
		    ...
		    tmpl->id.proto == x->id.proto &&
		    (tmpl->id.spi == x->id.spi || !tmpl->id.spi)) {
			/* TBD: spi should not be in check? */
			if (x->km.state == XFRM_STATE_VALID) {
				if (!best ||
				    best->km.dying > x->km.dying ||
				    (best->km.dying == x->km.dying &&
...
EXPORT_SYMBOL(xfrm_state_find_by_xfrm_policy);
