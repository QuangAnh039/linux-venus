...
#ifdef CONFIG_CA_KERNEL_HOOK
#include <soc/cortina-access/ca_kernel_hook_api.h>
#endif
...
static void dump_xfrm_state(struct xfrm_state *x)
{
	...
	pr_info("id: daddr=%08x-%08x-%08x-%08x, spi=0x%08x, proto=0x%02x\n",
		ntohl(x->id.daddr.a6[0]), ntohl(x->id.daddr.a6[1]),
		ntohl(x->id.daddr.a6[2]), ntohl(x->id.daddr.a6[3]),
		ntohl(x->id.spi), x->id.proto);
	pr_info("sel: daddr=%08x-%08x-%08x-%08x/%u, dport=%u, dport_mask=%u\n",
		ntohl(x->sel.daddr.a6[0]), ntohl(x->sel.daddr.a6[1]),
		ntohl(x->sel.daddr.a6[2]), ntohl(x->sel.daddr.a6[3]),
		x->sel.prefixlen_d, ntohs(x->sel.dport), ntohs(x->sel.dport_mask));
	pr_info("     saddr=%08x-%08x-%08x-%08x/%u, sport=%u, sport_mask=%u\n",
		ntohl(x->sel.saddr.a6[0]), ntohl(x->sel.saddr.a6[1]),
		ntohl(x->sel.saddr.a6[2]), ntohl(x->sel.saddr.a6[3]),
		x->sel.prefixlen_s, ntohs(x->sel.sport), ntohs(x->sel.sport_mask));
	pr_info("     family=%u, proto=%u, ifindex=%d\n",
		x->sel.family, x->sel.proto, x->sel.ifindex);
	pr_info("mark.v=0x%08x, mark.m=0x%08x\n", x->mark.v, x->mark.m);
	pr_info("tfcpad=0x%08x, genid=0x%08x\n", x->tfcpad, x->genid);
	pr_info("props: reqid=%u, mode=%u, replay_window=%u, aalgo=%u, ealgo=%u, calgo=%u\n",
		x->props.reqid, x->props.mode, x->props.replay_window,
		x->props.aalgo, x->props.ealgo, x->props.calgo);
	pr_info("       flags=0x%02x, family=%u, header_len=%u, trailer_len=%u, extra_flags=0x%08x\n",
		x->props.flags, x->props.family, x->props.header_len, x->props.trailer_len,
		x->props.flags);
	pr_info("       saddr=%08x-%08x-%08x-%08x\n",
		ntohl(x->props.saddr.a6[0]), ntohl(x->props.saddr.a6[1]),
		ntohl(x->props.saddr.a6[2]), ntohl(x->props.saddr.a6[3]));

	if (x->aalg) {
		pr_info("aalg: name=(%s), key_len=%u, trunc_len=%u\n",
			x->aalg->alg_name, x->aalg->alg_key_len, x->aalg->alg_trunc_len);
		print_hex_dump(KERN_INFO, "aalg: key=", DUMP_PREFIX_NONE, 32, 1, x->aalg->alg_key,
			       x->aalg->alg_key_len / 8, false);
	}

	if (x->ealg) {
		pr_info("ealg: name=(%s), key_len=%u\n",
			x->ealg->alg_name, x->ealg->alg_key_len);
		print_hex_dump(KERN_INFO, "ealg: key=", DUMP_PREFIX_NONE, 32, 1, x->ealg->alg_key,
			       x->ealg->alg_key_len / 8, false);
	}

	if (x->calg) {
		pr_info("calg: name=(%s), key_len=%u\n",
			x->calg->alg_name, x->calg->alg_key_len);
		print_hex_dump(KERN_INFO, "calg: key=", DUMP_PREFIX_NONE, 32, 1, x->calg->alg_key,
			       x->calg->alg_key_len / 8, false);
	}

	if (x->aead) {
		pr_info("aead: name=(%s), key_len=%u, icv_len=%u\n",
			x->aead->alg_name, x->aead->alg_key_len, x->aead->alg_icv_len);
		print_hex_dump(KERN_INFO, "aead: key=", DUMP_PREFIX_NONE, 32, 1, x->aead->alg_key,
			       x->aead->alg_key_len / 8, false);
	}

	if (x->encap) {
		pr_info("encap: encap_type=0x%04x, encap_sport=%u, encap_dport=%u, encap_oa=%08x-%08x-%08x-%08x\n",
			x->encap->encap_type,
			ntohs(x->encap->encap_sport), ntohs(x->encap->encap_dport),
			ntohl(x->encap->encap_oa.a6[0]), ntohl(x->encap->encap_oa.a6[1]),
			ntohl(x->encap->encap_oa.a6[2]), ntohl(x->encap->encap_oa.a6[3]));
	}

	if (x->coaddr) {
		pr_info("coaddr=%08x-%08x-%08x-%08x\n",
			ntohl(x->coaddr->a6[0]), ntohl(x->coaddr->a6[1]),
			ntohl(x->coaddr->a6[2]), ntohl(x->coaddr->a6[3]));
	}

	pr_info("replay: oseq=%u, seq=%u, bitmap=0x%08x\n",
		x->replay.oseq, x->replay.seq, x->replay.bitmap);

	if (x->replay_esn)
		pr_info("replay_esn: oseq=%u, seq=%u, oseq_hi=%u, seq_hi=%u, replay_window=%u, bmp_len=%u\n",
			x->replay_esn->oseq, x->replay_esn->seq,
			x->replay_esn->oseq_hi, x->replay_esn->seq_hi,
			x->replay_esn->replay_window, x->replay_esn->bmp_len);

	pr_info("xflags=0x%08x, replay_maxage=%u, replay_maxdiff=%u\n",
		x->xflags, x->replay_maxage, x->replay_maxdiff);
	pr_info("stats: replay_window=%u, replay=%u, integrity_failed=%u\n",
		x->stats.replay_window, x->stats.replay, x->stats.integrity_failed);
	pr_info("curlft.bytes=%llu, curlft.packets=%llu\n", x->curlft.bytes, x->curlft.packets);

	if (x->type)
		pr_info("type.proto=%u, type.flags=0x%02x\n", x->type->proto, x->type->flags);

	pr_info("inner_mode: encap=%u, flags=0x%x, fammily=%u\n",
		x->inner_mode.encap, x->inner_mode.flags, x->inner_mode.family);

	pr_info("inner_mode_iaf: encap=%u, flags=0x%x, family=%u\n",
		x->inner_mode_iaf.encap, x->inner_mode_iaf.flags, x->inner_mode_iaf.family);

	pr_info("outer_mode: encap=%u, flags=0x%x, family=%u\n",
		x->outer_mode.encap, x->outer_mode.flags, x->outer_mode.family);

	if (x->security) {
		pr_info("security: ctx_doi=%u, ctx_alg=%u, ctx_len=%u, ctx_sid=%u\n",
			x->security->ctx_doi, x->security->ctx_alg,
			x->security->ctx_len, x->security->ctx_sid);
	}
}
...
static void dump_xfrm_policy(struct xfrm_policy *p)
{
	...
	pr_info("index=%u, type=%u, action=%u, flags=0x%02x, family=0x%04x, xfrm_nr=%u, priority=%u\n",
		p->index, p->type, p->action, p->flags, p->family, p->xfrm_nr, p->priority);

	pr_info("selector: daddr=%08x-%08x-%08x-%08x/%u, dport=%u, dport_mask=%u\n",
	...
	pr_info("          saddr=%08x-%08x-%08x-%08x/%u, sel.sport=%u, sel.sport_mask=%u\n",
	...
	for (i = 0; i < p->xfrm_nr; i++) {
		...
		pr_info("  encap_family=%u, reqid=%u, mode=%u, share=%u\n",
			p->xfrm_vec[i].encap_family, p->xfrm_vec[i].reqid, p->xfrm_vec[i].mode,
			p->xfrm_vec[i].share);
	}
}
#endif /* CONFIG_CA_KERNEL_HOOK */
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if ((ca_kh_hook & CA_KH_HOOK_IPSEC) == 0)
		goto leave_ipsec_add;

	if (ca_kh_debug & CA_KH_DEBUG_TUNNEL) {
		pr_info("%s: dir=%d\n", __func__, p->dir);
		dump_xfrm_policy(xp);
	}
	if ((p->dir == XFRM_POLICY_IN || p->dir == XFRM_POLICY_OUT) &&
	    xp->xfrm_nr != 0 && xp->action == XFRM_POLICY_ALLOW) {
		...
		tmp_xp = xfrm_policy_construct(net, p, attrs, &err, extack);
		...
		rev_xp = get_policy_by_partial_policy((p->dir == XFRM_POLICY_IN) ?
						      XFRM_POLICY_OUT : XFRM_POLICY_IN, tmp_xp);
		if (rev_xp) {
			if (ca_kh_debug & CA_KH_DEBUG_TUNNEL) {
				pr_info("%s: FOUND REVERSED xfrm_policy!\n", __func__);
				dump_xfrm_policy(rev_xp);
			}
		}

		if (rev_xp && rev_xp->action == XFRM_POLICY_ALLOW) {
			xfrm_state_find_by_xfrm_policy(xp, &x);
			xfrm_state_find_by_xfrm_policy(rev_xp, &rev_x);

			if (x) {
				if (ca_kh_debug & CA_KH_DEBUG_TUNNEL) {
					pr_info("%s: FOUND xfrm_state! dir=%d!\n",
						__func__, p->dir);
					dump_xfrm_state(x);
				}
			}
			if (rev_x) {
				if (ca_kh_debug & CA_KH_DEBUG_TUNNEL) {
					pr_info("%s: FOUND REVERSED xfrm_state!\n", __func__);
					dump_xfrm_state(rev_x);
				}
			}

			if (x && rev_x && ca_kernel_hook_ops.kho_ipsec_add) {
				if (ca_kh_debug & CA_KH_DEBUG_TUNNEL)
					pr_info("%s: Add IPSec tunnel!\n", __func__);

				if (p->dir == XFRM_POLICY_IN)
					ca_kernel_hook_ops.kho_ipsec_add(0, x, xp, rev_x, rev_xp);
				if (p->dir == XFRM_POLICY_OUT)
					ca_kernel_hook_ops.kho_ipsec_add(0, rev_x, rev_xp, x, xp);
			}
		}

		security_xfrm_policy_free(tmp_xp->security);
		kfree(tmp_xp);
	}
leave_ipsec_add:
#endif /* CONFIG_CA_KERNEL_HOOK */
			   ...
			   struct netlink_ext_ack *extack)
{
	...
	if (!delete) {
		...
		if (p->dir == XFRM_POLICY_IN || p->dir == XFRM_POLICY_OUT) {
			struct xfrm_policy *tmp_xp, *rev_xp = NULL;
			struct xfrm_selector tmp_selector;
			struct xfrm_userpolicy_info tmp_pinfo;
			xfrm_address_t tmp_addr;
			int i;

			/* use info from xp to generate reversed xfrm_policy */

			tmp_pinfo.priority = xp->priority;
			tmp_pinfo.index = xp->index;
			tmp_pinfo.sel = xp->selector;
			tmp_pinfo.lft = xp->lft;
			tmp_pinfo.action = xp->action;
			tmp_pinfo.flags = xp->flags;
			tmp_pinfo.sel.family = xp->family;
			tmp_xp = xfrm_policy_construct(net, &tmp_pinfo, attrs, &err, extack);

			/* reverse the data of xp->selector */
			tmp_selector = tmp_xp->selector;
			tmp_selector.daddr = tmp_xp->selector.saddr;
			tmp_selector.saddr = tmp_xp->selector.daddr;
			tmp_selector.dport = tmp_xp->selector.sport;
			...
			rev_xp = get_policy_by_partial_policy((p->dir == XFRM_POLICY_IN) ?
							      XFRM_POLICY_OUT : XFRM_POLICY_IN,
							      tmp_xp);
			if (rev_xp) {
				if (ca_kh_debug & CA_KH_DEBUG_TUNNEL) {
					pr_info("%s: FOUND REVERSED xfrm_policy! Delete IPSec tunnel!\n",
						__func__);
					dump_xfrm_policy(rev_xp);
				}

				if (ca_kernel_hook_ops.kho_ipsec_delete) {
					if (p->dir == XFRM_POLICY_IN)
						ca_kernel_hook_ops.kho_ipsec_delete(0, xp, rev_xp);
					if (p->dir == XFRM_POLICY_OUT)
						ca_kernel_hook_ops.kho_ipsec_delete(0, rev_xp, xp);
				}
			}
		}
#endif /* CONFIG_CA_KERNEL_HOOK */
