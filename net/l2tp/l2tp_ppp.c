			    ...
			    int sockaddr_len, int flags)
{
	...
	if ((ca_kh_hook & CA_KH_HOOK_L2TP) && ca_kernel_hook_ops.kho_l2tp_add) {
		...
		/* update info stored by l2tp_tunnel_create() */
		if (sockaddr_len == sizeof(struct sockaddr_pppol2tp)) {
			tunnel->is_ipv6 = false;
			tunnel->local_ip.s_addr = tunnel->sock->sk_rcv_saddr;
			tunnel->peer_ip.s_addr = tunnel->sock->sk_daddr;
			tunnel->local_udp_port = ntohs(inet->inet_sport);
			tunnel->peer_udp_port = ntohs(inet->inet_dport);
			...
			tunnel->local_udp_port = ntohs(inet->inet_sport);
			tunnel->peer_udp_port = ntohs(inet->inet_dport);
		} else if (sockaddr_len ==
				sizeof(struct sockaddr_pppol2tpin6)) {
			tunnel->is_ipv6 = true;
			tunnel->local_ip6 = tunnel->sock->sk_v6_rcv_saddr;
			tunnel->peer_ip6 = tunnel->sock->sk_v6_daddr;
			tunnel->local_udp_port = ntohs(inet->inet_sport);
			tunnel->peer_udp_port = ntohs(inet->inet_dport);
		} else if (sockaddr_len ==
