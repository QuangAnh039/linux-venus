		       ...
		       struct l2tp_tunnel_cfg *cfg, struct l2tp_tunnel **tunnelp)
{
	...
	if (cfg) {
		...
		if (cfg->local_ip6 && cfg->peer_ip6) {
			tunnel->is_ipv6 = true;
			tunnel->local_ip6 = *cfg->local_ip6;
			tunnel->peer_ip6 = *cfg->peer_ip6;
		} else {
#endif
			tunnel->is_ipv6 = false;
			tunnel->local_ip = cfg->local_ip;
			tunnel->peer_ip = cfg->peer_ip;
#if IS_ENABLED(CONFIG_IPV6) /* stupid code due to checkpatch.pl */
		}
#endif
		tunnel->local_udp_port = cfg->local_udp_port;
		tunnel->peer_udp_port = cfg->peer_udp_port;
		tunnel->use_udp_checksums = cfg->use_udp_checksums;
		tunnel->udp6_zero_tx_checksums = cfg->udp6_zero_tx_checksums;
		tunnel->udp6_zero_rx_checksums = cfg->udp6_zero_rx_checksums;
	}
