...
int devinet_ioctl(struct net *net, unsigned int cmd, struct ifreq *ifr)
{
	...
	case SIOCSIFDSTADDR:	/* Set the destination address */
		...
		if (ca_kh_debug & CA_KH_DEBUG_TUNNEL)
			pr_info("%s: SIOCSIFDSTADDR, ifa->ifa_address=%pI4\n",
				__func__, &ifa->ifa_address);
		if (ca_kernel_hook_ops.kho_pppoe_update)
			ca_kernel_hook_ops.kho_pppoe_update(0, dev);
#endif
		break;
			 ...
			 void *ptr)
{
...
#ifdef CONFIG_CA_KERNEL_HOOK
	if (ca_kh_debug & CA_KH_DEBUG_INTERFACE)
		pr_info("%s: dev->name=%s event=%lu\n", __func__, dev->name, event);
#endif
	...
	case NETDEV_UP:
		...
		if ((dev->flags & IFF_LOOPBACK) == 0) {
			...
			memset(&ip_addr, 0, sizeof(struct __ca_ip_address));
			ip_addr.afi = __CA_IPV4;

			in_dev_for_each_ifa_rcu(ifa, in_dev) {
				if (ca_kh_debug & CA_KH_DEBUG_INTERFACE) {
					pr_info("%s: ifa->ifa_label=%s, ifa->ifa_local=%pI4, ifa->ifa_address=%pI4\n",
						__func__, ifa->ifa_label,
						&ifa->ifa_local,
						&ifa->ifa_address);
				}
				ip_addr.ip_addr.ipv4_addr = ntohl(ifa->ifa_local);
			...
			if (ca_kernel_hook_ops.kho_l3_intf_add)
				ca_kernel_hook_ops.kho_l3_intf_add(0, dev, &ip_addr, 0);
#else
			if (ca_kernel_hook_ops.kho_l3_intf_add)
				ca_kernel_hook_ops.kho_l3_intf_add(0, dev, &ip_addr, 0, 0);
#endif
		}
	...
	case NETDEV_DOWN:
#ifdef CONFIG_CA_KERNEL_HOOK
		if (ca_kernel_hook_ops.kho_l3_route_del_ipv4_ifdown)
			ca_kernel_hook_ops.kho_l3_route_del_ipv4_ifdown(0, dev);

		if (ca_kernel_hook_ops.kho_l3_route_del_ipv6_ifdown)
			ca_kernel_hook_ops.kho_l3_route_del_ipv6_ifdown(0, dev);

		/* handle PPPOE interface deletion in kho_pppoe_delete() */
		if (ca_kernel_hook_ops.kho_l3_intf_delete && dev->type != ARPHRD_PPP)
			ca_kernel_hook_ops.kho_l3_intf_delete(0, dev);
#endif
		ip_mc_down(in_dev);
		break;
...
#ifdef CONFIG_CA_KERNEL_HOOK
		if (ca_kh_debug & CA_KH_DEBUG_INTERFACE)
			pr_info("%s: dev->name=%s, type=%d\n", __func__, dev->name, dev->type);

		if (dev->type == ARPHRD_TUNNEL6) {
			if ((ca_kh_hook & CA_KH_HOOK_IP6_TUNNEL) &&
			    ca_kernel_hook_ops.kho_ip6tnl_delete_intf_only)
				ca_kernel_hook_ops.kho_ip6tnl_delete_intf_only(0, dev);
		}

		if (dev->type == ARPHRD_NONE) {
			if ((ca_kh_hook & CA_KH_HOOK_IP6_TUNNEL) &&
			    ca_kernel_hook_ops.kho_nat46_delete_intf_only)
				ca_kernel_hook_ops.kho_nat46_delete_intf_only(0, dev);
		}
#endif
