...
#ifdef CONFIG_CA_KERNEL_HOOK
		if (ca_kh_hook & CA_KH_HOOK_GRE && tunnel->dev->ca_tunnel_id_valid &&
		    tpi->proto == htons(ETH_P_TEB)) {
			skb->ca_offload_info.egress_ifindex = tunnel->dev->ca_tunnel_id;
			if (ca_kernel_hook_ops.kho_offload_l2gre)
				ca_kernel_hook_ops.kho_offload_l2gre(0, skb);
		}
#endif
